...
**Last manifest update:** 2026-07-28 · App SW version: v357
...
- **Forwarding a named share no longer misfiles answers** · `shipped 2026-07-28` · `SW v357` · the v333 fix above named the link's OWN recipient (`shareName`) — correct for a returning recipient, wrong the moment a link is FORWARDED. Koy sent Haley's named link to a designer and hit two bugs sharing one root cause: the share's LABEL (who the link was *made for*) stood in for *who is typing now*. (1) The name box **pre-filled "Haley"** (a `useEffect` on `shareName`), so an unedited submit would have filed the designer's answers under Haley — the same misattribution family as the Kweller relabeling. (2) The banner told every visitor "You previously submitted answers as Haley." Both now key off a **per-device** record (`he_qsub_{jobId}_{shareId}`, written on successful submit): the name box fills ONLY from this browser's own prior submit (a forwarded link opens blank), "You previously submitted…" shows ONLY to the device that actually did, and anyone else sees a neutral "Some of these were already answered by Haley — answer anything still open; put your own name at the bottom and your answers are filed under you." Display + prefill only; no write-path or data-shape change (per-question `answeredBy` attribution was already correct).
